...
const RecentOrdersTable: React.FC<{ orders: RecentOrder[] }> = ({ orders }) => (
  <TableContainer>
    <Card.Body>
      <Card.Title className="text-secondary mb-3">Recent Orders</Card.Title>
      <table className="table table-sm table-borderless">
        <thead>
          <tr>
            <th style={{ color: '#7c6f63' }}>ID</th>
            <th style={{ color: '#7c6f63' }}>Customer</th>
            <th style={{ color: '#7c6f63' }}>Total</th>
            <th style={{ color: '#7c6f63' }}>Status</th>
          </tr>
        </thead>
        <tbody>
          {orders.map((order, index) => (
            <tr key={index}>
              ...
              <td>{order.Total.toFixed ? order.Total.toFixed(2) : Number(order.Total).toFixed(2)}</td>
              <td>
      ...
      {orders.length === 0 && <p className="text-center text-muted mt-4">No recent orders data available.</p>}
    </Card.Body>
  </TableContainer>
);
  ...
  return (
    <DashboardContainer>
      ...
      <Row className="mb-4">
        ...
        <Col md={6} lg={3}>
          <StatisticCard
            title={`Total Orders (API) ${showOrdersReport ? '▲' : '▼'}`}
            value={ordersData!.totalOrdersFromAPI.toLocaleString()}
            icon="bi-box-seam"
            color="#4CAF50"
            isClickable={true}
            isActive={showOrdersReport}
            onClick={toggleOrdersReport}
          />
        </Col>
      ...
      <Row className="mb-5">
        <Col lg={6}>
          <RecentOrdersTable orders={ordersData!.recentOrders} />
        </Col>
        <Col lg={6}>
